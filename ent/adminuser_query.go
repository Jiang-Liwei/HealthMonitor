...
type AdminUserQuery struct {
	...
	withRoles  *AdminUserRoleQuery
	withLogs   *AdminLogQuery
...
func (auq *AdminUserQuery) QueryRoles() *AdminUserRoleQuery {
	query := (&AdminUserRoleClient{config: auq.config}).Query()
...
func (auq *AdminUserQuery) QueryLogs() *AdminLogQuery {
	query := (&AdminLogClient{config: auq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := auq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (auq *AdminUserQuery) WithRoles(opts ...func(*AdminUserRoleQuery)) *AdminUserQuery {
	query := (&AdminUserRoleClient{config: auq.config}).Query()
...
func (auq *AdminUserQuery) WithLogs(opts ...func(*AdminLogQuery)) *AdminUserQuery {
	query := (&AdminLogClient{config: auq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (auq *AdminUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AdminUser, error) {
	...
	if query := auq.withRoles; query != nil {
		if err := auq.loadRoles(ctx, query, nodes,
			func(n *AdminUser) { n.Edges.Roles = []*AdminUserRole{} },
			func(n *AdminUser, e *AdminUserRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	if query := auq.withLogs; query != nil {
		if err := auq.loadLogs(ctx, query, nodes,
			func(n *AdminUser) { n.Edges.Logs = []*AdminLog{} },
			func(n *AdminUser, e *AdminLog) { n.Edges.Logs = append(n.Edges.Logs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (auq *AdminUserQuery) loadRoles(ctx context.Context, query *AdminUserRoleQuery, nodes []*AdminUser, init func(*AdminUser), assign func(*AdminUser, *AdminUserRole)) error {
	...
	query.Where(predicate.AdminUserRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(adminuser.RolesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.admin_user_roles
		if fk == nil {
			return fmt.Errorf(`foreign-key "admin_user_roles" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "admin_user_roles" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (auq *AdminUserQuery) loadLogs(ctx context.Context, query *AdminLogQuery, nodes []*AdminUser, init func(*AdminUser), assign func(*AdminUser, *AdminLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*AdminUser)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.AdminLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(adminuser.LogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.admin_user_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "admin_user_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "admin_user_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
